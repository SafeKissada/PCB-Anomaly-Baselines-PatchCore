"""สร้างภาพผลลัพธ์จาก scores_{split}.npz ที่ run_patchcore.py เซฟไว้ —
...
Generates result images from scores_{split}.npz saved by run_patchcore.py —
...
Usage:
    ...
    (configure paths in RUN.py, same as run_patchcore.py)
"""
import sys
import logging
import pandas as pd
...
logger = logging.getLogger("visualize_patchcore")
...
def _load_scores(split: str, cfg) -> dict:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"ไม่พบ {path} — รัน scripts/run_patchcore.py ก่อนเพื่อสร้างไฟล์นี้\n"
            f"/ {path} not found — run scripts/run_patchcore.py first to create it."
        )
...
def _load_threshold(cfg) -> dict:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"ไม่พบ {path} — รัน scripts/run_patchcore.py ก่อน\n"
            f"/ {path} not found — run scripts/run_patchcore.py first."
        )
